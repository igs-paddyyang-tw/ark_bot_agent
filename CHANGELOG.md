...
原本啟動通知只有一行「🔄 Bot 已啟動（ark-bot-agent vX）」——太短，
使用者收到分不清是版本更新、例行重啟還是出問題重啟。

改為多行卡片：明說「例行啟動，非故障」+ 版本 + 時間 + 常駐池 agent 數 +
backend，並加一句「若你沒有手動重啟或升版，才需要留意」。用 `parse_mode="HTML"`
（只用 `<b>`，值皆為可控的版號/數字，不含使用者輸入 → 無 1.3.2 的標籤風險）。
...
bot 的 `/health` 原本只看 TG updater 有沒有在跑，看不到「agent 起來了、
TG 也 polling，但**人格沒載入 / mcp 空 / SOUL 不見 / backend 不在**」——
那時服務層一切正常（`status: ok`、`telegram: polling`）。這正是 team 側
preflight 記過的形狀（1.7.21）：**回報「沒問題」的指標與「根本沒執行」的
指標在畫面上是同一個值**，而 bot 側連前者都沒有。

- 新增 `preflight.py`（對齊 team 側 `preflight.py`，bot 是單 agent chat 服務
  故簡化掉多 instance 迴圈）：`@probe` 裝飾器 + `ProbeContext` + `Report`
- **4 個 P1 探針**（都是 deterministic、服務層看不出來的靜默故障）：
  - `agent_json`：`.kiro/agents/*.json` 無 BOM、可解析、`file://` 路徑存在
  - `mcp_json`：`mcp.json` 存在、可解析、mcpServers 非空
  - `chat_steering`：chat 來源 agent 的 SOUL.md 存在可讀（無它人格退預設）
  - `cli_backend`：kiro-cli backend 可用（chat spawn 靠它，不在則每則訊息都失敗）
- `run_and_write()` on_start 執行（`bot/main.py`，失敗**絕不阻擋啟動**），
  寫 `state/preflight.json`；`/health` 與 `/api/v1/health` 讀它
- **三態可分辨**（對齊 skills_verify）：`never_checked`（沒跑過，不算 degraded）/
  `error`（檔案壞了，不算 degraded）/ `ok`｜`degraded`（真跑過，P1 失敗才 degraded）
- 守門 `test_preflight.py` 16 條（病例回放 5 / 探針隔離 2 / health 接線 5 /
  report 語意 3 / 健康基線 1）。反證三方向全紅

> 🔴 **BOM 病例的斷言收緊到「刪掉開頭 3 bytes」而非通用 JSON 錯誤** ——
> `json.loads` 對 BOM 的錯誤訊息本身就含 'BOM'，只驗 `'BOM' in detail` 的話
> 移除專門的 BOM 檢查後測試照樣綠（team 側 1.7.21 同型坑）。
>
> 💡 實機驗證（paddy-bot editable）：塞 BOM → 重啟 → 抓到「1/4 失敗 agent_json」→
> 移除 → 重啟 → 4 探針全綠自癒。degraded 每次由 preflight.json 重算。
...
消費端希望使用者在快答模式就知道「這則要不要花錢」。原本各部署靠在
`SOUL.md` 手寫一段成本表達成 —— 那應該是套件預設行為，不該每個部署各寫一份
（**手寫第二份必然漂移**，本套件記過太多次）。

### 🔴 但「引擎／成本是架構事實」這個前提不成立

提案原本要把「LLM API（有成本）」與「kiro-cli（零 API 費用）」**寫死**進模式表，
理由是「chat 一定走 LLM provider、agent/team 一定走 CLI」。實查兩處設定：

```
LLMConfig.provider  = gemini | openai | anthropic   （且 env LLM_PROVIDER 可覆寫）
BackendConfig.cli   = auto | kiro | claude | agy    （且 env CLI_BACKEND 可覆寫）
```

**兩個都是設定值。** 某部署設 `cli: claude` 時，寫死的表格就在說謊 ——
而這段是**講給使用者聽的**（他要據此決定用哪個模式），比日誌說謊更糟。

### 🔴 「零 API 費用」也是誤導

kiro-cli 是訂閱制；team daemon 啟動就印 `Cost guard active: $30.00/day`，
還有 `record_spend` 工具 —— **CLI 那條路本身就有成本概念**。
準確的說法是「不計入 API 用量」，不是「零費用」。

### 作法：`_engine_labels()`，沿用 `_mode_labels()` 的既有原則

那個函式當初就是為了「不硬編別的部署的人名」而抽出來的。引擎名同理：

| 模式 | 引擎／成本（動態） |
|---|---|
| 💬 快答 | `{provider}／{model_for("chat")} API`，**每則都計費** |
| 👑 Agent · ⚔️ 團隊 | `{get_available_backend()} CLI`，**不計入 API 用量** |

- 模型走 `model_for("chat")` —— 與實際呼叫端同一個解析順序，不自己再解一次
- `cli: auto` 由 `get_available_backend()` 解析成**真的會跑的那個**（它也處理 env 覆寫）
- 🔴 讀不到設定時退回泛稱 `("LLM API", "CLI")`，**絕不填具體名字**
  （沿用 `_mode_labels()`：填錯的名字比不填更糟）

另補一條行為指引：被問到成本時據實說明，並講明 CLI 有訂閱與日額上限
—— **不是推銷切模式**。

### 守門 `tests/ark_bot_agent/test_mode_engine_disclosure.py`（5 條，三層）

① 行為層 —— **parametrize 兩組設定**（`gemini/kiro` vs `openai/claude`），
表格要跟著變；② fallback 不得吐出具體引擎名；
③ 掃描層用 ast 取 `_build_mode_table` 的字串常值，禁止出現引擎名字面值
（不用整檔比對 —— 那會被本檔這些說明註解命中）。

⚠️ **行為層第一版是空的**：斷言寫 `chat in table`，而下面那條行為指引 bullet
也帶了同樣的變數，所以把表格欄位改回寫死之後它仍然為 True
（反證只紅 2 條，兩條 parametrize 都沒紅）。收斂成「只驗表格那三列」後
反證變 **4 紅**。

> 💡 **斷言範圍太寬 = 空守門。** 而它與寫對的守門在畫面上完全一樣（都是綠的），
> 只有反證能區分。
...
`agent/cli.py` 的常駐 agent 橫幅寫的是：

```python
log.info("  ⭐ %s: persistent=true, skip_resume=false", name)
```

`skip_resume=false` 從來不是變數。不論 `agents.yaml` 設什麼都印 `false`，
而**這行是唯一能讓人看出該設定有沒有生效的地方**。

實測情境：把某個常駐 agent 改成 `skip_resume: true` 並重啟，
橫幅照樣印 `false` —— 但設定其實已經生效
（`AgentProcess._build_cmd()` 組出的命令確實不含 `--resume`）。
只有追進 `AgentProcess` 才知道。

> 🔴 **這比沒有這行 log 更糟：它會讓人回頭去改一個本來就對的設定。**
> 同型於本套件記過的 `doctor.sh` 印錯 `alive` 標籤、
> `/api/status` 的 `mcp_loaded` 回 None、`start.py` 顯示 8000 實際綁 8088。
> **顯示與實際必須同源。**

`persistent=true` 沒有這個問題（在該分支下必為真），
但一併改成印變數 —— 避免下次有人動了條件又出現同樣的謊。

### 守門 `tests/ark_bot_agent/test_startup_log_truthful.py`（3 條，兩層）

| 層 | 內容 |
|---|---|
| 行為層 | 真的跑 `start_all_agents()` 比對 log 與設定，**parametrize 兩個方向** |
| 掃描層 | 用 `ast` 取 log 的 format string，禁止再寫死旗標值 |

掃描層刻意不用整檔字串比對 —— 那會被說明註解命中而變成常駐假警報。

**反證**：退回舊寫法 → 2 紅（行為層 `True` 那條 + 掃描層）。
`[False]` 那條不紅是正確的 —— 舊的寫死值在 `skip_resume: false` 的部署上
碰巧正確，**這正是它能長期潛伏的原因**，也證明只驗一個方向會被完全騙過。
